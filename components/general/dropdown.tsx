...
export const DropDownViewer = ({ parentClass, displayName, name,totalValue, coinUrl, className, disableAddressDisplay }: { parentClass?: string, displayName?: string, name: string,totalValue?:string,  address?: string, coinUrl?: CoinsURL, className?: string, disableAddressDisplay?: boolean }) => <>
...
const Dropdown = ({ selected, list, toTop = false, photo = false, totalValue = false, nameActivation = false, onSelect, className, loader = false, disableAddressDisplay = false, parentClass = '', childClass = '', displayName, onChange }: { disableAddressDisplay?: boolean,  parentClass?: string, className?: string, toTop?: boolean, photo?: boolean,totalValue?: boolean, selected: DropDownItem, list: Array<DropDownItem>, nameActivation?: boolean, onSelect?: Dispatch<DropDownItem>, onChange?: Function, loader?: boolean, childClass?: string, displayName?: string }) => {
        ...
        <div className={`relative ${parentClass} `}>
            <div ref={expectRef} onClick={() => list?.length > 0 ? setOpen(!isOpen) : null} className={`flex ${className || ''} ${loader ? 'justify-center' : 'justify-between'} items-center border dark:border-darkSecond rounded-xl py-2 px-3 cursor-pointer`}>
                {!loader ? <div className={`truncate flex items-center gap-2`}>
                    {photo && <img src={`/icons/${selected.photo ? selected.photo : ""}.png`} className={`rounded-full w-10 h-10 bg-light dark:bg-greylish`} />}
                    {DropDownViewer({ name: selected.name, totalValue:selected.totalValue, address: selected?.address ?? selected?.amount, coinUrl: selected?.coinUrl, className: selected?.className, disableAddressDisplay: disableAddressDisplay, displayName })}
                </div> : <Loader />}
                ...
                <ul id="ala" className="flex flex-col overflow-y-auto" style={list.length > 5 ?
                    ...
                    {list?.filter((w) => {
                        ...
                        return <Li ref={(ref) => { liArrRef.current[i] = ref }} key={w.name + i} className={childClass} onClick={() => {
                            ...
                            {DropDownViewer({ name: w?.name,totalValue: selected.totalValue,address: w?.address ?? w?.amount, coinUrl: w?.coinUrl, className: w?.className, disableAddressDisplay })}
                        </Li>
                    }
                    )}
                </ul>
