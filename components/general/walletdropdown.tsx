...
import { IAccount, IMultiwallet, IUser } from "firebaseConfig";
...
export const WalletDropdown = ({ selected, onChange }: { selected: string, onChange: (accounts: IMultiwallet[]) => void }) => {
    ...
    const [selectedAccounts, setAccounts] = useState<IAccount[]>((data as IAccount[]) ?? [])

    useEffect(() => {
        if (data) {
            setAccounts(data as IAccount[])
        }
    }, [data])
    ...
    return <div className="relative">
        <div onClick={() => setOpen(!isOpen)} className="font-normal px-2 sm:px-2 py-2 rounded-xl cursor-pointer bg-white dark:bg-darkSecond flex space-x-1 items-center justify-center" ref={exceptRef}>
        ...
        {isOpen && <div ref={divRef} className="absolute min-w-[150%] rounded-xl sm:-right-0 -bottom-1 translate-y-full shadow-xl z-50">
            <ul>
                {data && data.map((w) => {
                    return <WalletDropdownItem key={w.id} w={w as IAccount} onChange={onChange} selectedAccount={selectedAccounts} setAccount={setAccounts} />
                })}
            </ul>
        </div>
        }
...
const WalletDropdownItem = ({ w, selectedAccount, setAccount, onChange }: { w: IAccount, selectedAccount: IAccount[], setAccount: Dispatch<React.SetStateAction<IAccount[]>>, onChange: (wallets: IMultiwallet[]) => void }) => {


    return <li key={w.address} onClick={() => {
        const list = (s: IAccount[]) => {
