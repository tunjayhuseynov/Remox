...
const Sidebarlist = ({ showbar }: { showbar: boolean }) => {
    ...
    return <>
        <ul className={`pt-10`}>
            ...
            {blockchain.name !== 'solana' && <NavLink to="/dashboard/choose-budget?page=lend-and-borrow" customPath="/dashboard/lend-and-borrow" className={({ isActive }) => `${isActive ? 'text-primary' : ''}`}>{({ isActive }) => <Li text={"Lend and Borrow"} showbar={showbar}><BorrowSVG active={isActive} darkMode={darkMode} />{showbar && 'Lend - Borrow'}</Li>}</NavLink>}
            {blockchain.name !== 'solana' && <NavLink to="/dashboard/automations" className={({ isActive }) => `${isActive ? 'text-primary' : ''}`}>{({ isActive }) => <Li text={"Recurring"} showbar={showbar}><AutomationsSVG active={isActive} darkMode={darkMode} />{showbar && 'Recurring'}</Li>}</NavLink>}
            {showbar && <><div className="w-full border-b my-4"></div>
                ...
                <div className="flex gap-8 items-center justify-center pr-10 py-3 pb-6">
                    ...
                    <img onClick={darkModee} src={!darkMode ? '/icons/navbar/dark.png' : '/icons/navbar/dark_active.png'} className="w-6 h-6 self-center cursor-pointer" alt='dark' />
