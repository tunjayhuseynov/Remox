...
function SubBudgets({ item }: { item: ISubbudgetORM }) {
  ...
  return <div className=" border-b py-4 w-full flex justify-between items-center">
    ...
    <div className="text-greylish flex items-start gap-1 text-sm">
      <img src={`/icons/currencies/${token.coinUrl}.svg`} className="w-5 h-5 rounded-full" alt="" /><span className="text-red-600">{used}</span>/  <img src={`/icons/currencies/${token.coinUrl}.svg`} className="w-5 h-5 rounded-full" alt="" /><span className="text-black">{0}</span>  /  <img src="/icons/currencies/celodollar.svg" className="w-5 h-5" alt="" /><span className="text-greylish">{total - used}</span>
    </div>
