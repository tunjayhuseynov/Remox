...
const Contributors = () => {
    ...
    return <div>
        <div className="flex flex-col  pb-5 gap-10">
            <div className="flex justify-between items-center w-full">
                ...
                <Button className="text-xs sm:text-base !py-2 !px-6 rounded-2xl" onClick={() => setAddTeamModal(true)}>Add Team</Button>
                    {contributors.length > 0 && <Button className="text-xs sm:text-base  !py-2 !px-6 rounded-2xl" onClick={() => setAddMemberModal(true)}>Add People</Button>}
                </div>
            </div>
        ...
        {selectbar !== "Team" ? <div className="w-full  pt-4 pb-6">
            <div id="header" className="hidden sm:grid grid-cols-[30%,30%,1fr] lg:grid-cols-[17%,15%,19%,19%,17%,23%] rounded-xl bg-light  dark:bg-dark  sm:mb-5">
            ...
            <div>
                {contributors.map(w => w && w.members && w.members.length > 0 ? <Fragment key={w.id}><TeamContainer {...w} selectbar={selectbar} /></Fragment> : undefined)}
                {contributors.map(w => w && (!w?.members || w?.members?.length === 0) ? <Fragment key={w.id}><TeamContainer {...w} selectbar={selectbar} /></Fragment> : undefined)}
