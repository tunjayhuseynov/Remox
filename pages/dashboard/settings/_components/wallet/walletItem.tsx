...
import {
  Remove_Account_From_Individual,
  Remove_Account_From_Organization,
  Update_Account_Image,
  Update_Account_Name,
} from "redux/slices/account/thunks/account";
...
function WalletItem({ item }: { item: IAccountORM }) {
  const { handleSubmit: handleAddOwnerSubmit, register: registerAddOwner } =
    useForm();
  const { handleSubmit: handleThresholdSubmit, register: registerThreshold } =
    useForm();
  ...
  const [addOwnerImageType, setAddOwnerImageType] = useState<"image" | "nft">(
    "image"
  );
  ...
  const deleteWallet = async () => {
    try {
      ...
      if (accountType === "individual") {
        if (!individual)
          return ToastRun(<>Cannot find your session</>, "error");
        await dispatch(
          Remove_Account_From_Individual({
            account: item,
  ...
  const addNewOwner = async (data: {
    ...
    try {
      if (!item.provider)
        return ToastRun(
          <>Cannot find the account's multisig provider</>,
          "error"
        );
      await addOwner(
        item,
      ...
      ToastRun(
        <>Be sure there is no another owner addition transaction</>,
        "error"
      );
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        >
          <div
            className={`bg-white dark:bg-darkSecond rounded-md shadow-custom p-5 pr-9 grid grid-cols-[2.5%,25%,20%,25%,7.5%,1fr]`}
          >
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-greylish dark:text-white text-xs mx-2">
            ...
            <div className={`flex items-center justify-center`}>
              <span className={`${!item.multidata && "hidden"} text-sm`}>
                {item.multidata?.threshold.sign} out of{" "}
                {item.multidata?.owners.length} owners
              </span>
            </div>
            <div className="flex items-center justify-center">
              <div className="flex items-center relative">
                {item.members.slice(0, 3).map((member, index) => (
                  <Avatar
                    ...
                    alt={member.name}
                    src={
                      individual?.accounts?.[0].id === member.address
                        ? individual.image?.imageUrl ??
                          makeBlockie(individual?.name || "random")
                        : member.image?.imageUrl ??
                          member.image?.nftUrl ??
                          makeBlockie(member?.name || "random")
                    }
                  />
                ))}
                {item.members.length > 3 && (
                    <div className="w-[1.5rem] h-[1.5rem] bg-[#E5E5E5] dark:bg-greylish absolute z-5 !bottom-[50] left-[2.4rem] flex items-center justify-center !text-xs rounded-full border-[1px]">
                        <span className="text-xs font-medium text-[#707070] dark:text-white">
                            +{item.members.length - 3}
                        </span>
                    </div>
                )}
            ...
            <div
              className="flex space-x-3 justify-end items-center"
              onClick={(event) => event.stopPropagation()}
            >
          ...
          {item.multidata && (
            <AccordionDetails
              className="!ml-[9%] bg-white dark:bg-darkSecond shadow-custom mt-1 rounded-sm"
              onClick={(event) => event.stopPropagation()}
            >
              {item.members.map((s, i) => (
                <div
                  key={s.id}
                  className={`${
                    i !== item.members.length - 1 &&
                    "border-b dark:border-gray-500 py-3"
                  }`}
                >
        ...
        >
          <div className="flex flex-col space-y-8 items-center">
            ...
            <div className="flex justify-center items-center space-x-4">
              <Button
                version="second"
                className="border-2  w-[7rem] h-[2.7rem] !px-1 !py-0"
                onClick={() => {
              ...
              <Button
                className="w-[7rem] h-[2.7rem] !px-1 !py-0"
      ...
      {addOwnerModal && (
        <Modal
          onDisable={setAddOwnerModal}
          animatedModal={false}
          disableX={true}
          className={"!pt-6 !w-[30%]"}
        >
          <form
            onSubmit={handleAddOwnerSubmit(AddOwner)}
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              <TextField
                label="Owner Name"
                type="text"
                {...registerAddOwner("name", { required: true })}
                placeholder="E.g: Jessy"
                className="border w-full py-3 text-base rounded-md px-3 dark:bg-darkSecond"
              />
            </div>
            <div className="flex flex-col gap-1">
              <TextField
                label="Owner Address"
                type="text"
                {...registerAddOwner("address", { required: true })}
            ...
            <div className="flex flex-col gap-1">
              <TextField
                label="Owner Email"
                type="email"
                {...registerAddOwner("mail", { required: false })}
                placeholder="remox@remox.io"
                className="border w-full py-3 text-base rounded-md px-3 dark:bg-darkSecond"
              />
            </div>
      ...
      {changeThresholdModal && (
        <Modal
          onDisable={setChangeThresholdModal}
          animatedModal={false}
          disableX={true}
          className={"!pt-6 !w-[30%]"}
        >
          <form
            onSubmit={handleThresholdSubmit(ChangeThreshold)}
            className="flex flex-col gap-7"
          >
            <div className="flex flex-col gap-1">
              <TextField
                defaultValue={item.multidata?.threshold.sign}
                label="Threshold"
                type="number"
