...
const Details = () => {
    ...
    const walletAddresses = useMemo(() => info?.walletAddress.map(s => ({ name: s })) ?? [], [info])
            ...
            <div className="w-full shadow-custom px-5 py-14 rounded-xl flex flex-col items-center justify-center">
                ...
                {tx && info && !notFound ? <div className="flex flex-col sm:grid sm:grid-cols-3 py-5 gap-14 w-full">
                    ...
                    {info.walletAddress.length === 1 ?
                        ...
                        :
                        <Dropdown
                            label="Choose to copy an address"
                            className="h-[4.688rem] bg-greylish bg-opacity-10"
                            runFn={val => async () => {
                                if (val.name) {
                                    await window.navigator.clipboard.writeText(val.name)
                                    ToastRun(<>`Copied ${val.name} to clipboard`</>, "success")
                                }
                            }}
