...
export async function FirestoreRead<DataType extends {}>(
  collection: string,
  document: string
) {
  try {
...
export function useFirestoreRead<DataType>(
  collection: string,
  document: string
) {
...
export function useFirestoreReadMultiple<DataType extends {}>(
  collectionName: string,
  queries: { firstQuery: string; condition: WhereFilterOp; secondQuery: any }[]
) {
  const [data, setData] = useState<DataType[]>();
...
export async function FirestoreReadAll<DataType extends {}>(
  collectionName: string
) {
  let list: DataType[] = [];
  const querySnapshot = await getDocs(collection(db, collectionName));
  querySnapshot.forEach((doc) => {
    list.push(doc.data() as DataType);
  });
  return list;
}

export async function FirestoreReadMultiple<DataType extends {}>(
  collectionName: string,
  queries: { firstQuery: string; condition: WhereFilterOp; secondQuery: any }[]
) {
  const q = query(
...
export function useFirestoreSearchField() {
  ...
  const search = async function <DataType>(
    collectionName: string,
    queries: {
      field: string;
    ...
    const q = query(
      ref,
      ...queries.map((s) => where(s.field, s.indicator, s.searching))
    );
