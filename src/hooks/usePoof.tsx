...
export default function usePoof(provingSystem: number) {
    ...
    const transfer = async (currency: PoofCoinsName, amount: number | string, recipient: string) => {
        try {
            ...
            const res = await poof.withdraw(pk, currency, amountIn, debt, recipient, "https://privatecelo.com")
    ...
    const balance = async (currency: "CELO_v2" | "cUSD_v2" | "cEUR_v2" | "cREAL_v2") => {
        const account = await poof.getLatestAccount(pk.toLowerCase(), currency, Events.current)
    ...
    const pastEvents = async (currency: "CELO_v2" | "cUSD_v2" | "cEUR_v2" | "cREAL_v2") => {
        const poolMatch = await poof.poolMatch(currency);
