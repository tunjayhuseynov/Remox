...
export default function Create({ setCreateModal }: { setCreateModal: Dispatch<boolean> }) {
    ...
    return <div className="flex flex-col gap-4 mt-[-2rem]">
        ...
        <div className="flex flex-col overflow-y-auto max-h-[75vh] space-y-5">
            <span className="opacity-35 pl-3">Owners</span>
            ...
            </div>
            {
                Array(ownerAmount).fill(' ').map((e, i) => {
                    return <div className="w-full" key={e}>
                        <input type="text" onChange={(e) => { owners.current[i] = e.target.value }} className="border p-3 rounded-md border-black outline-none w-full dark:bg-darkSecond" placeholder="0xabc..." />
                    </div>
                })
            }
            <div className="cursor-pointer text-center text-greylish opacity-80 px-3 dark:text-white dark:opacity-100" onClick={() => { setOwnerAmount(ownerAmount + 1) }}>+ Add Owner</div>
            <div className="flex flex-col space-y-8">
                <div>
                    <span className="text-greylish opacity-35 pl-3">Name</span>
                    <input type="text" className="border p-3 rounded-md border-black outline-none w-full dark:bg-darkSecond" value={name} onChange={(e) => { setName(e.target.value) }} required/>
                </div>
                <div>
                    <span className="text-greylish opacity-35 pl-3">Signatures required to execute TXs</span>
                    <input type="text" className="border p-3 rounded-md border-black outline-none w-full dark:bg-darkSecond" value={sign} onChange={(e) => { if (!isNaN(+e.target.value)) setSign(+e.target.value || undefined) }} required/>
                </div>
                <div>
                    <span className="text-greylish opacity-35 pl-3">Signatures required to change MultiSig properties</span>
                    <input type="text" className="border p-3 rounded-md border-black outline-none w-full dark:bg-darkSecond" value={internalSign} onChange={(e) => { if (!isNaN(+e.target.value)) setInternalSign(+e.target.value || undefined) }} required/>
