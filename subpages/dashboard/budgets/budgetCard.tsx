...
function BudgetCard({ setEditBudget, setDelBudget, item, id }: { item: IBudgetItem, id: number, setEditBudget: React.Dispatch<boolean>, setDelBudget: React.Dispatch<boolean> }) {
    ...
    return <>
        <BudgetDetails item={item} setNotify={setNotify} openNotify={openNotify} />
        <div key={id} className="relative" >
            <div className="flex space-x-3 justify-end absolute top-0 right-1 z-[9]" >
            ...
            <div onClick={() => { setNotify(!openNotify) }} className="rounded-xl shadow-lg px-4 py-4 bg-white transition-all dark:bg-darkSecond hover:transition-all hover:!bg-[#f0f0f0] dark:hover:!bg-[#131313]  hover:shadow-xl" >
                <div className="flex items-center justify-between w-full">
                    ...
                    <div className="flex items-center gap-5">
                        <div className="text-xl font-bold pr-6">{item.Percent}</div>
