...
const BudgetDetails = forwardRef<HTMLDivElement, IProps>(({ item, close, visibility }, ref) => {
    ...
    return <>
        <AnimatePresence>
            {visibility && <motion.div initial={{ x: "100%", opacity: 0.5 }} animate={{ x: 15, opacity: 1 }} exit={{ x: "100%", opacity: 0.5 }} transition={{ type: "spring", stiffness: 400, damping: 40 }} ref={ref} className="z-[97] blu fixed shadow-custom min-w-[50rem] h-[100vh] pr-1 overflow-y-auto overflow-x-hidden top-0 right-0 bg-white dark:bg-darkSecond cursor-default">
