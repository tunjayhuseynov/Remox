...
import { SelectContributors, updateMemberFromContributor } from "redux/slices/account/remoxData";

const EditMember = (
  props: IMember & {
    onCurrentModal: Dispatch<boolean>;
  ...
  const [value, setValue] = useState(
    props.usdBase ? "Pay with USD-based Amounts" : "Pay with Token Amounts"
  );
  ...
  //     e.preventDefault();
  //     const { memberName, memberSurname, amount, address, amount2 } =
  ...
  //             }
  //             if (
  //               props.paymantDate !== startDate.toISOString() ||
  //               props.paymantEndDate !== endDate.toISOString()
  //             ) {
  //               await updateTime(
  //                 props.taskId as string,
  //                 startDate.toISOString(),
  //                 selectedFrequency.type as DateInterval
  //               );
  //             }
  //             if (
  //               props.amount !== amountValue.trim() ||
  //               props.currency !== selectedWallet.name ||
  //               props.secondaryAmount !== amountValue2 ||
  //               props.secondaryCurrency !== selectedWallet2.name
  //             ) {
  ...
  //               if (selectedType) {
  //                 realMoney *=
  //                   balance[GetCoins[selectedWallet.name].name]?.tokenPrice ?? 1;
  //               }
  //               await allow(
  //                 GetCoins[selectedWallet.name].contractAddress,
  ...
  //                 if (selectedType) {
  //                   realMoney *=
  //                     balance[GetCoins[selectedWallet2.name].name]?.tokenPrice ??
  //                     1;
  //                 }
  //                 await allow(
  //                   GetCoins[selectedWallet2.name].contractAddress,
  //                   Contracts.Gelato.address,
  //                   realMoney.toString()
  //                 );
  //                 paymentList.push({
  ...
  //               const encodeAbi = (
  //                 await GenerateBatchPay(paymentList)
  //               ).encodeABI();
  ...
  //           secondaryCurrency: selectedWallet2?.name
  //             ? (selectedWallet2.name as CoinsName)
  //             : null,
  ...
  //         dispatch(
  //           changeSuccess({ activate: true, text: "Member updated successfully" })
  //         );
  //       } catch (error: any) {
  //         console.error(error);
  //         dispatch(
  //           changeError({
  //             activate: true,
  //             text: error?.message ?? "There was an error updating the member",
  //           })
  //         );
  //       }
  //     }
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (data) => {
    ...
    try {
      ...
      await editMember(props.teamId, props.id, newMember);
      dispatch(updateMemberFromContributor({ id: Team!.id!, member: newMember }))
                ...
                <div className="grid grid-cols-2 gap-x-10">
                  <div>
                    ...
                    <input
                      ...
                      className="border pl-2 rounded-md outline-none py-2  w-full dark:bg-darkSecond"
                      required
                    />
                  </div>
                  <div>
                ...
                <div className="flex flex-col">
                  ...
                  <div className="w-full ">
                    <Dropdown
                      ...
                      nameActivation={true}
                      className="!py-[0.6rem]  border !rounded-md "
                    />
                  </div>
                </div>
                <div className="flex flex-col ">
