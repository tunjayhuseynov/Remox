...
function paydropdown({ paymentname, value, setValue, className,className2, setSelectedType, onChangeType, selectedExecution, usdbase }: { paymentname: string[], value: string, setValue: Dispatch<SetStateAction<string>>, className?: string,className2?: string, setSelectedType?: Dispatch<SetStateAction<boolean>>, selectedExecution?: boolean, usdbase?: boolean, onChangeType?: (value: boolean) => void }) {
    ...
    return <div className="relative w-full">
        <div onClick={() => setOpen(!isOpen)} className={` ${className} w-full font-normal  px-2 sm:px-2 ${value ? 'py-3' : 'py-5'} text-base rounded-md bg-white dark:bg-darkSecond cursor-pointer bg-sec border  flex space-x-1  items-center justify-between`}>
        ...
        {isOpen && <div className={`${className2} absolute flex w-full  rounded-lg  -bottom-1 translate-y-full bg-white dark:bg-darkSecond  z-[9999]`}>
            <ul className="w-full ">
